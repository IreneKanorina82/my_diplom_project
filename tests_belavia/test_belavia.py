...
def test_settings_language_buttons(web_browser):
    ...
    for language_button in language_buttons:
        ...
        with allure.step(f'Проверка элемента на наличие'):
            check.is_true(language_button.is_presented(), f'Radio button is not presented')

        with allure.step(f'Проверка текста элемента на отображение'):
            check.is_true(language_button.is_visible(), f'Radio button text is not displayed')

        with allure.step(f'Проверка элемента на кликабельность'):
            check.is_true(language_button.is_clickable(), f'Radio button is not clickable')

        with allure.step(f'Проверка, что элемент выбран'):
            language_button.click()
            time.sleep(2)
            check.is_true(language_button.is_selected(), f'Radio button is not selected')
